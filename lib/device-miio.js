...
module.exports = class MiioDevice extends EventEmitter {
  ...
  async localSend(method, params, options = {}) {
    const result = await miioProtocol.send(
      this.address,
      method,
      params,
      options,
  ...
  async cloudSend(method, params, options) {
    const result = await miCloudProtocol.miioCall(
      this.id,
      method,
      params,
      options,
    ...
    try {
      ...
      for (const propChunk of propsChunks) {
        ...
        if (resultChunk.length !== propChunk.length) {
          throw new Error(
            `Result ${JSON.stringify(resultChunk)} and props ${JSON.stringify(
              propChunk,
  ...
  async getProperties(props) {
    const result = this._miotSpec
      ? await this.miotGetProperties(props)
      : await this.miioGetProperties(props);
    return result;
  }
