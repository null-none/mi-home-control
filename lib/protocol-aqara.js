...
const AQARA_IV = Buffer.from([
  0x17, 0x99, 0x6d, 0x09, 0x3d, 0x28, 0xdd, 0xb3, 0xba, 0x69, 0x5a, 0x2e, 0x6f,
  0x58, 0x56, 0x2e,
]);
...
class AqaraProtocol extends EventEmitter {
  ...
  getGatewayAddress(address) {
    return this.hasGatewayAddress(address)
      ? this._gatewaysByAddress.get(address)
      : {};
  }
  ...
  updateGatewayAddress(address, data) {
    const gatewayByAddress = Object.assign(
      this.getGatewayAddress(address),
      data,
    );
  ...
  async send(msg) {
    ...
    if (!address || !port) {
      throw new Error(
        `Gateway ${subDevice.parent} of subDevice ${sid} is missing address or port`,
      );
    }
    if (cmd === "write") {
      if (!key) {
        throw new Error(
          `Gateway ${subDevice.parent} is missing key when send write command`,
        );
      }
